...
test("the contact-map SVG is exported and social metadata uses the Pages URL", () => {
  assert.equal(existsSync(path.join(outputRoot, "dopamine-d2-contact-map.svg")), true);
...
test("the exported lab includes the functional-group replacement experience", () => {
  assert.match(indexHtml, /Replace one functional group/);
  assert.match(indexHtml, /3-Methoxy/);
  assert.match(indexHtml, /N-Methyl/);
  assert.match(indexHtml, /Tyramine/);
  assert.match(indexHtml, /Run again/);
});
